<template>
  <div class="edit" v-loading="loading">
    <el-page-header @back="goEditBack" content="编辑"> </el-page-header>
    <el-form
      :inline="true"
      :model="form"
      class="form-style1"
    ...
    >
      ...
      <el-form-item label="巡检类型">
        <el-select v-model="form.recordType" placeholder="巡检类型">
          ...
          <el-option label="年度检查" value="年度检查"></el-option>
    ...
    <el-tabs v-model="activeIndex" @tab-click="changeTab" type="border-card">
      <el-tab-pane
    ...
    <div class="button form-style1">
      ...
      <el-button @click="goEditBack">取消</el-button>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    getData() {
      ...
      inspectionDetail(this.id)
        .then(function (r) {
          if (r.data.code == 200) {
            data = r.data.data.dsmPtrRecord;
            // that.reportData = r.data.data;
            // that.basicsInfo = r.data.data.dsmPtrRecord;
            that.form.date = data.time;
            that.form.reRz = data.reRz;
            that.form.recordPerson = data.recordPerson;
            // that.form.principalPerson = [];
            that.form.type = data.type;
            that.form.recordType = data.recordType;
            that.form.weather = data.weather;
            that.form.entity = data.entity;
            that.reRz = data.reRz;
            if (data.principal !== "") {
              that.form.principalPerson = data.principal.split(",");
            }
            ...
            tab.forEach((item, ii) => {
              point.forEach((el, i) => {
                if (el.msps == item) {
                  el.result = el.result == 1 ? true : false;
                ...
                img.forEach((el) => {
                  if (item == el.imgAddress) {
                    that.allImageList[ii].push({
                      // url: "http://220.180.11.124:8081" + el.imgUrl,
                      url: that.baseUrl + el.imgUrl,
                    });
                    // that.imgList[ii].name = el.msps;
                    that.imgList[ii].urls.push(el.imgUrl);
                  }
                });
              }
            });
      ...
      //           };
      //           that.allImageList[j] = [];
      //           that.imgList[j] = { value: el.msps, urls: [] };
      //           j++;
      //         }
      //       });
    ...
    getImg() {
      this.fileList = [];
      if (this.allImageList[this.activeIndex].length > 0) {
        this.allImageList[this.activeIndex].forEach((el) => {
          this.fileList.push({ url: el.url });
        });
      } else {
        this.fileList = [];
      }
