<template>
  ...
  <div class="rainfallReport">
    ...
    <div class="reportTitle">{{ form.year }}年度逐日坝下水位报表</div>
    <el-table
      class="bigTable"
      :data="tableData"
      style="width: 100%"
    ...
    >
      ...
      <el-table-column label="月份及坝下水位(m)" align="center">
...
<script>
import { getUnderWaterLevelReport } from "@/http/api";
...
export default {
  ...
  methods: {
    getTableData() {
      let that = this;
      that.loading = true;
      getUnderWaterLevelReport(that.form.year, {})
        .then(function (r) {
          let data = [];
          if (r.data.code == 200) {
            data = r.data.data;
    ...
    exportExcel() {
      exportToExcel(this.form.year + "年度逐日坝下水位报表", "out-table-ylz");
    },
